---
name: retrospective
description: Run a retrospective on the current (or a given) work session — review what was journaled and what actually happened, then append one durable summary entry back to agent-journal.
...
1. **Pull this session's journal entries.** `journal_get` (or `agent-journal get`), scoped to the
...
2. **Review what actually happened this session** from your own memory of it — what was
   ...
   Journaled entries won't cover everything; this step fills the gaps between them. If a session
...
3. **Write one retrospective entry**, appended via `journal_append` with
