...
beforeEach(() => {
  tmpRoot = mkdtempSync(join(tmpdir(), 'agent-journal-session-'))
})
...
function writeStateFile(dir: string, content: string): void {
  mkdirSync(join(dir, '.agent-journal'), { recursive: true })
  writeFileSync(join(dir, '.agent-journal', 'session.json'), content)
}
