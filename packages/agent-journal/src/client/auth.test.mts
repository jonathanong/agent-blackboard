...
describe('Auth', () => {
  it('createCredentials() posts { name } using the admin token', async () => {
    ...
    try {
      const auth = new Auth({ baseUrl: fixture.baseUrl, adminToken: 'ag_admin_root_secret' })
      const result = await auth.createCredentials({ name: 'ci-bot' })
      expect(result).toEqual(created)
      expect(fixture.requests[0]!.headers.authorization).toBe('Bearer ag_admin_root_secret')
    } finally {
      await fixture.close()
    }
  })
