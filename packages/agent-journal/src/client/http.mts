...
import type { ClientConfig, GetRawQuery, JournalWireFormat } from './types.mjs'

/** Query params as they go on the wire — every value is a string. */
type WireQuery = Record<string, string>

/** Converts a typed `GET /journals` query into wire-format string params. */
export function buildJournalsQuery(query: GetRawQuery): WireQuery {
  const wire: WireQuery = {}
  if (query.sessionId !== undefined) wire.sessionId = query.sessionId
  if (query.agent !== undefined) wire.agent = query.agent
  if (query.archived !== undefined) wire.archived = String(query.archived)
  if (query.format !== undefined) wire.format = query.format
  return wire
}

const ACCEPT_HEADERS: Record<JournalWireFormat, string> = {
...
export function acceptHeaderFor(format: JournalWireFormat): string {
 ...
 * throwing `AgentJournalError` on any non-2xx status. Callers that need the
...
export async function rawRequest(
  ...
  if (!response.ok) {
    ...
    throw new AgentJournalError(
      `agent-journal request failed: ${options.method} ${path} -> ${response.status}`,
      response.status,
      body,
    )
  }
