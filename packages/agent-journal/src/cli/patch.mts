...
/**
 * `agent-journal patch <id> [--archived] [--data <json>]` for a single
 * patch, or `agent-journal patch --file <path>` to apply a batch from a
...
export async function runPatch(argv: string[], ctx: CliContext): Promise<void> {
  ...
  const journals = new Journals(journalConfigFromEnv(ctx.env))
  const result = await journals.patch(patches)
