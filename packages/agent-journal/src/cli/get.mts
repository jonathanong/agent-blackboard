import { fetchJournalsRaw } from '../client/stream.mjs'
...
import { journalConfigFromEnv } from './env.mjs'
import type { GetRawQuery, JournalWireFormat } from '../client/types.mjs'

const VALID_FORMATS: readonly JournalWireFormat[] = ['json', 'jsonl', 'markdown']

function parseFormatFlag(value: string | boolean | undefined): JournalWireFormat | undefined {
  if (value === undefined) return undefined
  if (typeof value === 'string' && (VALID_FORMATS as readonly string[]).includes(value)) {
    return value as JournalWireFormat
  }
  throw new CliError(`--format must be one of: ${VALID_FORMATS.join(', ')}`)
}

/**
 * `agent-journal get [--session-id] [--agent] [--archived] [--format]` —
...
export async function runGet(argv: string[], ctx: CliContext): Promise<void> {
  ...
  const config = journalConfigFromEnv(ctx.env)
  ...
  const response = await fetchJournalsRaw(config, query)
