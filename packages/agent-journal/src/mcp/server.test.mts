...
describe('createMcpServer', () => {
  it('lists exactly the three journal tools — no credential management', async () => {
  ...
  it('calls journal_append and returns the created entry as JSON text content', async () => {
    const entry = {
      id: 'a',
      sessionId: 's1',
      agent: 'claude-code',
  ...
  it('defaults missing arguments to {} and surfaces the resulting validation error', async () => {
    const client = await connectedClient({ baseUrl: 'http://h/', token: 't' })
    const result = await client.callTool({ name: 'journal_append' })
  ...
  it('returns isError: true for an unknown tool name', async () => {
    const client = await connectedClient({ baseUrl: 'http://h/', token: 't' })
    const result = await client.callTool({ name: 'journal_delete', arguments: {} })
    expect(result.isError).toBe(true)
    expect(textOf(result.content)).toBe('Unknown tool: journal_delete')
  })
})
