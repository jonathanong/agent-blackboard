...
import { JOURNAL_TOOLS } from './schemas.mjs'
import type { CallToolResult } from '@modelcontextprotocol/sdk/types.js'
import type { ClientConfig } from '../client/types.mjs'

const SERVER_INFO = { name: 'agent-journal', version: '0.0.0' }

/**
 * Builds the MCP server with the three journal tools registered, but does
...
export function createMcpServer(config: ClientConfig): Server {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: JOURNAL_TOOLS }))
...
/** Starts the MCP stdio server — the `agent-journal mcp` entrypoint. */
