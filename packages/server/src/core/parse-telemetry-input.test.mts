...
describe('parseTelemetryEntriesBody', () => {
  it('parses a single JSON object', async () => {
    const result = await parseTelemetryEntriesBody(
  ...
  it('defaults data to {} when omitted', async () => {
    const result = await parseTelemetryEntriesBody('{"sessionId":"s1","agent":"claude"}')
  ...
  it('parses a JSON array of entries', async () => {
    ...
    const result = await parseTelemetryEntriesBody(body)
  ...
  it('rejects an empty body', async () => {
    const result = await parseTelemetryEntriesBody(undefined)
  ...
  it('rejects invalid JSON and invalid NDJSON', async () => {
    const result = await parseTelemetryEntriesBody('not json at all\nstill not json')
    expect(result.ok).toBe(false)
  })

  it('rejects an entry missing sessionId', async () => {
    const result = await parseTelemetryEntriesBody('{"agent":"claude"}')
    expect(result.ok).toBe(false)
  })

  it('rejects an entry missing agent', async () => {
    const result = await parseTelemetryEntriesBody('{"sessionId":"s1"}')
    expect(result.ok).toBe(false)
  })

  it('rejects an entry with an empty sessionId', async () => {
    const result = await parseTelemetryEntriesBody('{"sessionId":"","agent":"claude"}')
    expect(result.ok).toBe(false)
  })
  ...
  it('rejects an entry whose data is an array', async () => {
    const result = await parseTelemetryEntriesBody(
      '{"sessionId":"s1","agent":"claude","data":[1,2]}',
    )
    expect(result.ok).toBe(false)
  })

  it('rejects a non-object item in an array body', async () => {
    const result = await parseTelemetryEntriesBody('[42]')
    expect(result.ok).toBe(false)
  })
})
