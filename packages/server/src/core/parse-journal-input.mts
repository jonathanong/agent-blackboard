...
import type { JournalEntryInput, RequestBody } from './types.mjs'

/** A validated entry ready to append — unlike `JournalEntryInput`, `data` is always populated (defaulted to `{}`), never `undefined`. */
...
export type ParseResult = { ok: true; entries: ParsedJournalEntry[] } | { ok: false; error: string }

function isValidEntryInput(value: unknown): value is JournalEntryInput {
...
/**
 * Parses the `POST /journals` body: one entry object, an array of entries,
 ...
 */
export async function parseJournalEntriesBody(body: RequestBody): Promise<ParseResult> {
