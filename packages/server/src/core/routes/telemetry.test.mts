...
import { handleTelemetryRoute } from './telemetry.mjs'
...
function baseRequest(overrides: Partial<HandlerRequest>): HandlerRequest {
  return { method: 'GET', path: '/telemetry', query: {}, headers: {}, ...overrides }
}

describe('handleTelemetryRoute', () => {
  ...
  describe('auth', () => {
    it('401s with no Authorization header', async () => {
      const response = await handleTelemetryRoute(baseRequest({}), store)
      expect(response.status).toBe(401)
    })

    it('401s for an admin-shaped token', async () => {
      const response = await handleTelemetryRoute(
        baseRequest({ headers: { authorization: 'Bearer atl_admin_alice_secret' } }),
        store,
      )
      expect(response.status).toBe(401)
    })

    it('401s for a telemetry token with the wrong secret', async () => {
      const response = await handleTelemetryRoute(
  ...
  it('404s for an unsupported method', async () => {
    const response = await handleTelemetryRoute(withAuth({ method: 'DELETE' }), store)
  ...
  describe('POST', () => {
    ...
    it('appends an array of entries', async () => {
      ...
      const response = await handleTelemetryRoute(withAuth({ method: 'POST', body }), store)
      const created = JSON.parse(await collect(response.body))
  ...
  describe('GET', () => {
    ...
    it('defaults to a JSON array', async () => {
      const response = await handleTelemetryRoute(withAuth(), store)
    ...
    it('supports format=jsonl', async () => {
      const response = await handleTelemetryRoute(withAuth({ query: { format: 'jsonl' } }), store)
      expect(response.headers['content-type']).toBe('application/x-ndjson')
    })

    it('supports format=markdown', async () => {
      const response = await handleTelemetryRoute(
        withAuth({ query: { format: 'markdown' } }),
        store,
      )
    ...
    it('400s on an invalid format', async () => {
      const response = await handleTelemetryRoute(withAuth({ query: { format: 'yaml' } }), store)
      expect(response.status).toBe(400)
    })

    it('filters by archived=true/false', async () => {
      const trueResponse = await handleTelemetryRoute(
        withAuth({ query: { archived: 'true' } }),
        store,
      )
      const falseResponse = await handleTelemetryRoute(
    ...
    it('400s on an invalid archived value', async () => {
      const response = await handleTelemetryRoute(withAuth({ query: { archived: 'maybe' } }), store)
      expect(response.status).toBe(400)
    })
  ...
  describe('PATCH', () => {
    it('merges data and sets archived', async () => {
      ...
      const response = await handleTelemetryRoute(withAuth({ method: 'PATCH', body }), store)
