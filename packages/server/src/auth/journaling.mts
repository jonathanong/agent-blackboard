...
import { parseJournalingToken } from './tokens.mjs'

/**
 * The minimal store surface this resolver needs — a structural subset of
 * `JournalStore`, kept local so `auth/**` never has to import from `store/**`.
 */
...
/**
 * Resolves an `Authorization` header value to a journaling `CredentialRecord`:
 * parses `ag_sk_<credId>_<secret>` (rejecting anything else, including
 ...
 */
export async function resolveJournalingCredential(
  authHeaderValue: string | undefined,
  store: CredentialLookup,
): Promise<CredentialRecord | undefined> {
  const presented = parseBearerToken(authHeaderValue)
  if (!presented) return undefined
  const parsed = parseJournalingToken(presented)
