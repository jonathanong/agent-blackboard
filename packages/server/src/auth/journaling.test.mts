...
import { generateJournalingToken } from './tokens.mjs'
import { resolveJournalingCredential, type CredentialLookup } from './journaling.mjs'
...
describe('resolveJournalingCredential', () => {
  it('resolves a valid token to its credential record', async () => {
    ...
    const resolved = await resolveJournalingCredential(`Bearer ${token}`, storeWith(record))
    expect(resolved).toEqual(record)
  })

  it('returns undefined when the Authorization header is missing', async () => {
    expect(await resolveJournalingCredential(undefined, storeWith(undefined))).toBeUndefined()
  })
  ...
  it('returns undefined for an admin-shaped token without a store lookup', async () => {
    ...
    const resolved = await resolveJournalingCredential('Bearer ag_admin_alice_secret', store)
  ...
  it('returns undefined when the credId is not found in the store', async () => {
    ...
    const resolved = await resolveJournalingCredential(`Bearer ${token}`, storeWith(undefined))
    expect(resolved).toBeUndefined()
  })
  ...
  it('returns undefined when the secret does not match the stored hash', async () => {
    ...
    const resolved = await resolveJournalingCredential(`Bearer ${tamperedToken}`, storeWith(record))
    expect(resolved).toBeUndefined()
  })
})
