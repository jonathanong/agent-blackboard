...
function parseAdminCredentials(env: AdminEnv): AdminCredentialEntry[] {
  ...
  try {
    const json = Buffer.from(env.ADMIN_CREDENTIALS, 'base64').toString('utf8')
 ...
 * from `ADMIN_CREDENTIALS`. Every configured entry is compared (no early
 ...
 * the header is missing/malformed, or `ADMIN_CREDENTIALS` is unset/invalid).
 */
