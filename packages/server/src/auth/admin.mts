...
function parseAdminCredentials(env: AdminEnv): AdminCredentialEntry[] {
  if (!env.AGENT_BLACKBOARD_ADMIN_CREDENTIALS) return []
  try {
    const json = Buffer.from(env.AGENT_BLACKBOARD_ADMIN_CREDENTIALS, 'base64').toString('utf8')
 ...
 * from `AGENT_BLACKBOARD_ADMIN_CREDENTIALS`. Every configured entry is compared (no early
 ...
 * the header is missing/malformed, or `AGENT_BLACKBOARD_ADMIN_CREDENTIALS` is unset/invalid).
 */
