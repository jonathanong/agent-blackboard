...
export function buildTelemetriesQuery(query: GetRawQuery): WireQuery {
...
const ACCEPT_HEADERS: Record<TelemetryWireFormat, string> = {
...
export function acceptHeaderFor(format: TelemetryWireFormat): string {
...
export async function rawRequest(
  ...
  if (!response.ok) {
    ...
    throw new AtelError(
      `atel request failed: ${options.method} ${path} -> ${response.status}`,
      response.status,
      body,
    )
  }
