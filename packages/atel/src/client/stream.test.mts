...
describe('fetchTelemetriesRaw', () => {
  it('defaults to the json Accept header and omits format from the query when unset', async () => {
    ...
    try {
      await fetchTelemetriesRaw({ baseUrl: fixture.baseUrl, token: 't' }, {})
      const request = fixture.requests[0]!
      expect(request.headers.accept).toBe('application/json')
      expect(request.url).toBe('/telemetry')
    } finally {
      await fixture.close()
    }
  })
