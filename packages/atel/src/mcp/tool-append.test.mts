...
describe('handleTelemetryAppend', () => {
  it('appends using the given sessionId/agent and returns the created entry', async () => {
    ...
    try {
      const result = await handleTelemetryAppend(
  ...
  it('defaults agent to claude-code and resolves sessionId when omitted', async () => {
    const fixture = await startHttpFixture((_req, res) => sendJson(res, 200, ENTRY))
    try {
      await handleTelemetryAppend({ data: {} }, { baseUrl: fixture.baseUrl, token: 't' })
  ...
  it('throws when data is missing or not an object', async () => {
    ...
    await expect(handleTelemetryAppend({}, config)).rejects.toThrow('"data" must be an object.')
    await expect(handleTelemetryAppend({ data: 'x' }, config)).rejects.toThrow(
