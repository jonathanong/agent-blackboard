...
describe('Sessions', () => {
  it('creates, lists, gets, and archives sessions', async () => {
    const session = { id: 's/1', parentSessionId: null, createdAt: 'now', archivedAt: null }
    ...
    try {
      ...
      expect(await sessions.create({ id: 's/1', parentSessionId: null })).toEqual(session)
      ...
      expect(JSON.parse(fixture.requests[3]!.body)).toEqual({ archived: true })
    } finally {
      await fixture.close()
    }
  })
