...
describe('Auth', () => {
  it('createCredentials() posts { name } using the admin token', async () => {
    const created = { id: 'c1', name: 'ci-bot', token: 'abb_sk_c1_secret', createdAt: 'now' }
    const fixture = await startHttpFixture((_req, res) => sendJson(res, 200, created))
    try {
      const auth = new Auth({ baseUrl: fixture.baseUrl, adminToken: 'abb_admin_root_secret' })
      const result = await auth.createCredentials({ name: 'ci-bot' })
      expect(result).toEqual(created)
      expect(fixture.requests[0]!.headers.authorization).toBe('Bearer abb_admin_root_secret')
    } finally {
      await fixture.close()
    }
  })
