...
```bash
npx -y agent-blackboard@0.6.0 sessions create root-123 --agent claude-code --version 1.0.13
npx -y agent-blackboard@0.6.0 sessions create worker-456 --parent-session-id root-123 \
  --agent claude-code --version 1.0.13
npx -y agent-blackboard@0.6.0 sessions patch worker-456 --data '{"branch":"fix/retry"}'
npx -y agent-blackboard@0.6.0 sessions patch worker-456 --data '{"repositories":["owner/name"]}'
npx -y agent-blackboard@0.6.0 append --session-id worker-456 '{"repositories":["owner/name"],"note":"found the failing edge case"}'
npx -y agent-blackboard@0.6.0 get --session-id worker-456 --format markdown
npx -y agent-blackboard@0.6.0 snapshot export --root-only --inactive-for-hours 8
npx -y agent-blackboard@0.6.0 snapshot partition --path /tmp/agent-blackboard-snapshot-<uuid>.jsonl \
  --cleanup-token <cleanup-token>
npx -y agent-blackboard@0.6.0 snapshot cleanup --path /tmp/agent-blackboard-snapshot-<uuid>.jsonl \
