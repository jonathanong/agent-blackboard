...
```bash
npx -y agent-blackboard sessions create root-123 --agent claude-code --version 1.0.13
npx -y agent-blackboard sessions create worker-456 --parent-session-id root-123 \
  --agent claude-code --version 1.0.13
npx -y agent-blackboard sessions patch worker-456 --data '{"branch":"fix/retry"}'
npx -y agent-blackboard append --session-id worker-456 '{"note":"found the failing edge case"}'
npx -y agent-blackboard get --session-id worker-456 --format markdown
```
