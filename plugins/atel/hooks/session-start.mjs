...
//
// Writes the live session id to `.atel/session.json` in the working
// directory. The published `atel` package's session resolution reads this
// file, so a new session (including after /clear, or a fresh Codex thread) starts a
// fresh telemetry stream with no manual reset — this is the reliable mechanism for
